...
struct tDB_ENTRY
{
    ...
    { "Interfaces.txt", "IID" }, { "Classes.txt", "CLSID" },
...
struct tGUIDNODE : public Container::NodeEx<Container::ListHT, tGUIDNODE>
{
    ...
    static void operator delete(PVOID _Ptr)
    {
        return (qfree(_Ptr));
    }
};
...
static const char szMainDialog[]
    = { "BUTTON YES* Continue\n" // 'Continue' instead of 'okay'
        ...
        "<#Skip code and import segments to make searching faster.\nUsually ok, but some times GUIDs are in code "
        "segments too, in particular Delphi executables. #"
                ...
                if (segment_t* pSegInfo = getnseg(i))
                {
                    ...
                    else
                    {
                        ...
                        while (pNode)
                        {
                            ...
                            ea_t ea = startEA;
                            while ((ea = find_binary(
                                        ea, endEA, pNode->szGUID, 0, (SEARCH_DOWN | SEARCH_NEXT | SEARCH_NOSHOW)))
                                != BADADDR)
                            {
                                msg("%08X %s\n", ea, pNode->szLabel);
                ...
                if ((szLine[0] && szLine[1] && szLine[2]) && (szLine[0] != '#'))
                {
                    ...
                    if (_snscanf(szLine, (sizeof(szLine) - 1), "%255s %255s", szGUID, szLabel) == 2)
                    {
                        ...
                        if (tGUIDNODE* pNode = new tGUIDNODE())
                        {
                            ...
                            // Label
                            qsnprintf(
                                pNode->szLabel, (sizeof(pNode->szLabel) - 1), "%s_%s", aDBFile[i].pszType, szLabel);
